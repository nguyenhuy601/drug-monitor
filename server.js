...
const connectMongo = require('./server/database/connect');//requires connect.js fileconst https = require("https")
...
// Bắt lỗi không được catch trong promise
process.on('unhandledRejection', (reason, promise) => {
    console.error('Unhandled Rejection at:', promise, 'reason:', reason);
    // Có thể log vào file hoặc DB
});

// Bắt lỗi không được catch ở cấp toàn cục
process.on('uncaughtException', (err) => {
    console.error('Uncaught Exception:', err.stack || err);
    // Có thể log vào file hoặc DB
    // process.exit(1); // Tùy chọn: restart server nếu muốn
});
